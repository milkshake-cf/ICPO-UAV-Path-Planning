...
%  Adaptive Grey Wolf Optimizer (AGWO) for UAV 3D Path Planning                 %
%                                                                                 %
%  Improvements over standard GWO:                                                %
%  1. Personal Best memory (pBest) - each agent remembers its best position       %
%  2. Adaptive nonlinear a-decay for exploration/exploitation balance             %
%  3. Fitness-weighted leader blending (Alpha/Beta/Delta)                          %
%_________________________________________________________________________________%
...
isInit = false;
while (~isInit)
    disp('Initializing AGWO population...');
    for i=1:nPop
...
disp('Starting AGWO optimization...');
...
%% Plot Results
disp('==================== AGWO OPTIMIZATION COMPLETE ====================');
disp(['Final Best Cost (AGWO) = ' num2str(GlobalBest.Cost)]);
...
title('AGWO: Adaptive Grey Wolf Optimizer - UAV Path Planning');

figure(2);
plot(BestCost, 'LineWidth', 2, 'Color', [0.8 0.2 0.2]);
xlabel('Iteration');
ylabel('Best Cost');
title('AGWO Convergence Curve');
grid on;

save('results/AGWO_results.mat', 'BestCost', 'GlobalBest', 'BestPosition');
disp('Results saved to results/AGWO_results.mat');
