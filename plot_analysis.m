...
%% Load data
load('results/batch_comparison_results.mat');  % SPSO, CPO, AGWO
spsocpoagwo = results;
...
allResults = struct();
allResults.SPSO = spsocpoagwo.SPSO;
allResults.CPO  = spsocpoagwo.CPO;
allResults.AGWO = spsocpoagwo.AGWO;
...
fprintf('Comparing each algorithm against AGWO\n');
...
agwo_fc = allResults.AGWO.finalCosts;
for a = [1,2,4,5]  % Compare SPSO, GWO, WOA, CPO vs AGWO
    alg = algs{a};
    other_fc = allResults.(alg).finalCosts;
    p = ranksum(agwo_fc, other_fc);
    sig = 'NO';
    interp = 'Not significantly different';
    if p < 0.01
        sig = 'YES (p<0.01)';
        if mean(agwo_fc) < mean(other_fc)
            interp = 'AGWO is significantly BETTER';
        else
...
fprintf('CPO vs AGWO:  p = %.6f\n', ranksum(allResults.CPO.finalCosts, allResults.AGWO.finalCosts));
