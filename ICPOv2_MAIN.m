...
%  ICPO v2: Improved CPO with Chaos + Levy Flight                                  %
%  Additional improvements over ICPO v1:                                           %
...
disp('Initializing ICPO v2 (Chaos + Levy)...');
...
%% ICPO v2 Main Loop
disp('Starting ICPO v2 optimization...');
...
disp('=== ICPO v2 COMPLETE ===');
disp(['Final Best Cost = ' num2str(GlobalBest.Cost)]);
BestPosition=SphericalToCart(GlobalBest.Position,model);
PlotSolution(BestPosition,model,0.95); title('ICPO v2: Chaos + Levy');
figure(2); plot(BestCost,'LineWidth',2); xlabel('Iteration'); ylabel('Best Cost');
title('ICPO v2 Convergence'); grid on;
save('results/ICPOv2_results.mat','BestCost','GlobalBest','BestPosition');
